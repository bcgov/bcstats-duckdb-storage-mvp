...
Groupings AS (
    SELECT
        ...
        EFF_DATE,
        END_DATE,
        -- Create a grouping key based on gaps in time, 28 is a number between 1 and 31. which allows people skip one month data in the combined file.
        SUM(CASE WHEN Prev_END_DATE IS NULL OR EFF_DATE > DATEADD(DAY, 1, Prev_END_DATE) THEN 1 ELSE 0 END)
            OVER (PARTITION BY STUDY_ID, POSTAL_CODE, STREET_LINE ORDER BY EFF_DATE) AS GroupKey
    FROM OrderedData
),
