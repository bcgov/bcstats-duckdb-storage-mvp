...
MonthlyData AS (
    SELECT
        ...
        -- Calculate lagged END_MONTH for comparison
        LAG(END_MONTH) OVER (
            PARTITION BY STUDY_ID, POSTAL_CODE, STREET_LINE
            ORDER BY EFF_MONTH
        ) AS Prev_END_MONTH,
        -- Calculate lead EFF_MONTH for alignment
        LEAD(EFF_MONTH) OVER (
            PARTITION BY STUDY_ID, POSTAL_CODE, STREET_LINE
            ORDER BY EFF_MONTH
        ) AS Next_EFF_MONTH
...
Groupings AS (
    SELECT
        ...
        EFF_MONTH,
        END_MONTH,
        -- Adjust END_MONTH based on gaps in months
        CASE
        ...
        SUM(CASE
            ...
            THEN 1 ELSE 0
        END) OVER (PARTITION BY STUDY_ID, POSTAL_CODE, STREET_LINE ORDER BY EFF_MONTH) AS GroupKey
    FROM MonthlyData
),
AggregatedPeriods AS (
    SELECT
        ...
        MAX(Adjusted_END_MONTH) AS EndMonth -- Maximum adjusted end month
    FROM Groupings
    GROUP BY STUDY_ID, POSTAL_CODE, STREET_LINE, GroupKey
)
SELECT
    STUDY_ID,
    POSTAL_CODE,
    ...
    INTO DEV.FCT_HEALTH_CLIENT_ADDRESS_ORDERED
FROM AggregatedPeriods
ORDER BY STUDY_ID, EFF_MONTH;
