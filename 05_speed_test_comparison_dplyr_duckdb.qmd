...
for (csv in csvs$path) {
  results_csv = filter(results, path == csv) |>
    select(starts_with("result"))
  
  unique(results_csv$result_dplyr)
  unique(results_csv$result_duckplyr)
  
  
  |>
    as.list()
  results_csv = do.call(c, results_csv)
  results_csv = map(results_csv, as_tibble)
  View(unique(results_csv))
}

x=results |>
  select(path, trial, starts_with("result")) |>
  arrange(path, trial)

identical(results$result_dplyr[[1]], results$result_duckplyr[[1]])

|>
  head(5) |>
  pull(result_dplyr)

identical(x[[1]], x[[2]], x[[3]], x[[4]], x[[5]])
