...
read_duckDB = function(name = "t1", files, delim = ',', quote = "\"", header = T, transaction = T) {
    tryCatch({
      ...
      output = time_function(duckdb::duckdb_read_csv, params = list(conn = db, name = name, files = files, delim = delim, quote = quote, header = header, transaction = transaction, nrow.check = params$n_max))
