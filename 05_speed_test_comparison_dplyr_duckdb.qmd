...
```{r}
queries$r$query_duckplyr = map_chr(queries$r$query_dplyr, ~str_replace(., "df |>\n", "df |>\n    duckplyr::as_duckplyr_tibble() "))

queries$duckDB$query_duckDB = map_chr(queries$r$query_dplyr, ~str_replace(., "df |>", 'tbl(db, "t1") ')) |> paste(" |>\n  collect()")
...
for (i in 1:nrow(trials)) {
  ...
  # load df into R
  message("loading R")
  c(df, load_time_df) %<-% time_function(fn = read_csv, params = list(file = csvs_i$path, quote = "\"", n_max = params$n_max))
  
  # load duckDB
  message("loading duckDB")
  db = duckdb::dbConnect(duckdb::duckdb())
  c(db_return_code, load_time_duckDB) %<-% time_function(fn = read_duckDB, params = list(name = "t1", files = csvs_i$path, header = T, transaction = T))
