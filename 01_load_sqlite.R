...
pacman::p_load(odbc, tidyverse, config, DBI, dbplyr,nanoarrow, arrow, duckdb, RSQLite)
...
bcstats_sqlite_con <- DBI::dbConnect(RSQLite::SQLite(),
                                file.path(test_csv_folder, "bcstats_db.sqlite"))

## test connection by write a small table to it
# dbWriteTable(bcstats_con, "iris", iris)

# list all tables in the database
dbListTables(bcstats_sqlite_con)
...
load_csv_save_db(bcstats_sqlite_con,
                 file_path = csv_files[2],
                 table_name = table_names[2])
tictoc::toc()
# 53.91 sec elapsed
...
dbListFields(bcstats_sqlite_con, "Inventory_20230630")

dbListFields(bcstats_sqlite_con, "BC_Stat_CLR_EXT_20230525")
